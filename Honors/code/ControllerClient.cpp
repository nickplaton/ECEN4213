// g++ -std=c++11 -o ControllerClient ControllerClient.cpp -lwiringPi -pthread
...
void read_socket(){
	...
	while(1){
		...
		if (bytes_read == 4) {
			memcpy(&incomingRadius, buffer, sizeof(float));
			//printf("received radius: %f\n", incomingRadius);
		}
		int sign = 1;
		if (incomingRadius < 0){
			sign = 1;
		...
		float radius = sign * (500-499*(abs(incomingRadius)));
		if(abs(radius)>440){
			radius = 0;
		}
		cout<<radius<<endl;
		...
		/**/if (radius > 1){
			cout<<"1"<<endl;
			sp = int(speed * (radius + b/2)/radius);
			r = int(radius);
			movement(sp, r);
		}
		else if (radius < -1){
...
/*void readData(){
	...
	while(true){
		...
		serialGetchar(kobuki);
		//Read the bytes containing the bumper, wheel drop, and cliff sensors.
		bumper = serialGetchar(kobuki);
		drop = serialGetchar(kobuki);
		cliff = serialGetchar(kobuki);

		//Read through the bytes between the cliff sensors and the button sensors.
		serialGetchar(kobuki);
		serialGetchar(kobuki);
		serialGetchar(kobuki);
		serialGetchar(kobuki);
